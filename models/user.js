...
class User {
  ...
  /** Get list of companies applied to by user
   *
   * - username: username applying for job
   **/
  static async getAppliedCompanies(username) {
    ...
    if (!user) throw new NotFoundError(`No user: ${username}`);
    const res = await db.query(`SELECT a.job_id, j.title, c.name FROM applications AS a LEFT JOIN jobs AS j ON j.id = a.job_id LEFT JOIN companies AS c ON c.handle = j.company_handle WHERE a.username=$1`, [username]);
    return res.rows;
  }
